...
class TwitterBot:
  ...
  def senddm(self, i, dmsender, status, postid=None, rttime=None):
    ...
    if status == 'sent':
      message = {'sent': 'El mensaje se ha publicado, puedes consultarlo aquí '+url}
    elif status == 'notsent':
      message = {'notsent' : 'El mensaje no se ha publicado, inténtalo de nuevo más tarde :('}
    else: message = {'wrong attachment' : 'El mensaje no se ha publicado porque contiene un vídeo o GIF, solo pueden adjuntarse imágenes :(.'}
